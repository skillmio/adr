...
# ===========================================
#   Rudder Installation Script for AlmaLinux 10
# ===========================================
...
function prompt_user_inputs() {
  # Keeping prompts intact as requested
...
function get_latest_version() {
  version=$(curl -fsSL https://api.github.com/repos/henrygd/rudder/releases/latest \
    | grep '"tag_name"' \
    | cut -d '"' -f4 \
    | sed 's/^v//')
...
function ensure_rudder_user() {
  id rudder &>/dev/null || useradd -r -s /usr/sbin/nologin rudder
}
...
function detect_architecture() {
  ...
  OS=$(uname -s)
  TARBALL="rudder_${OS}_${ARCH}.tar.gz"
  TMP_PATH="${TMP_DIR}/${TARBALL}"
}

function download_rudder() {
  DOWNLOAD_URL="https://github.com/henrygd/rudder/releases/download/v${version}/${TARBALL}"
...
function install_rudder() {
  mkdir -p "${INSTALL_DIR}/rudder_data"
  tar -xzf "$TMP_PATH" -C "$INSTALL_DIR"
  chmod +x "${INSTALL_DIR}/rudder"
  chown -R rudder:rudder "$INSTALL_DIR"
}

function configure_systemd() {
  tee /etc/systemd/system/rudder-hub.service > /dev/null <<EOF
[Unit]
Description=Rudder Hub Service
After=network.target

[Service]
ExecStart=${INSTALL_DIR}/rudder serve --http 0.0.0.0:${PORT}
...
EOF
  ...
  systemctl enable --now rudder-hub.service
}

function configure_nginx() {
  tee /etc/nginx/conf.d/rudder.conf > /dev/null <<EOF
server {
    listen 80;
    server_name ${SERVER_IP} ${ACCESS_URL};

    access_log /var/log/nginx/rudder_access.log;
    error_log /var/log/nginx/rudder_error.log;
...
info_msg "${MSG_SAVE_SERVICE}: rudder-hub.service"
